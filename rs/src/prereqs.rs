...
mod prereqs_tests {
    ...
    use crate::programs::bfl_prereq::{BflPrereqProgram, CompleteArgs, UpdateArgs};
    ...
    fn enroll() {
        ...
        let signer = read_keypair_file("bfl-wallet.json").expect("Couldn't find wallet file");
        let prereq = BflPrereqProgram::derive_program_address(&[b"prereq", signer.pubkey().to_bytes().as_ref()]);
        ...
        let transaction = BflPrereqProgram::complete(
    ...
    fn update() {
        ...
        // Now we can invoke the "complete" function
        let transaction = BflPrereqProgram::update(
            &[&signer.pubkey(), &prereq, &system_program::id()],
            &args,
            Some(&signer.pubkey()),           
            &[&signer],
